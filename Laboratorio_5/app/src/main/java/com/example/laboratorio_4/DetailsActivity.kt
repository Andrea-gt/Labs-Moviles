...
class DetailsActivity : AppCompatActivity() {
    ...
    private fun checkPermission(permission:String,requestCode:Int){
        if(ContextCompat.checkSelfPermission(this@DetailsActivity,permission)==PackageManager.PERMISSION_DENIED){
            ActivityCompat.requestPermissions(this@DetailsActivity, arrayOf(permission),requestCode)
        } else{
            Toast.makeText(this@DetailsActivity,"Permiso otorgado",Toast.LENGTH_SHORT).show()
        }
    }
    ...
    override fun onRequestPermissionsResult(
        ...
        if(requestCode==CAMERA_PERMISSION_CODE){
            if(grantResults.isNotEmpty()&& grantResults[0]==PackageManager.PERMISSION_GRANTED){
                Toast.makeText(this@DetailsActivity,"Permiso de Camara Otorgado", Toast.LENGTH_SHORT).show()
            } else{
